...
begin
  ...
  Label1.Caption:='До окончания теста:' + IntToStr(M) + ':' + IntToStr(S);
...
var
  i:integer;
  begin
  Timer1.Enabled:=true;   //включаем таймер времени
  Button1.Visible:=false; //убираем кнопку начать
  Button3.Visible:=true;  //включаем кнопку закончить
  Button3.Enabled:=false; //делаем её недействительной
  Button2.Enabled:=true;  //делаем действительнйо кнопку с.в.
  DataModule11.ans.First;
  DBText1.Visible:=true; //показываем вопрос
...
begin
  ...
  n:=n+1;
  if n < DataModule11.quest.RecordCount then GroupBox1.Caption:='Вопрос ' + IntToStr(n+1) + ' из ' + IntToStr(DataModule11.quest.RecordCount)
  else Label1.Caption:='Тест закончен,' + ' Набранные баллы : ' + IntToStr(p) ;
